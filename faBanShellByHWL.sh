...
# if OK 就发版  else notOK 就不发版

#   #   #  #  #   #  #  #  #  #  #  #  # 
#    This is server of 11.24.201.223   #
#   #   #  #  #   #  #  #  #  #  #  #  #

FABANMA="发版吗？"
url="192.168.0.86:8080/xx/223";
...
OK_notOK=$(awk 'NF{a=$0}END{print a}' yy.txt|cut -d " " -f5)
...
then	
	echo "要发版，请注意发版！！！"
		...
		cd /acme_dbgo/acme_backup
		zip -r acme_$(date +"%Y-%m-%d-%H-%M").zip /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps/acme

		cd /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps
		...
		mkdir acme 
		cd /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps/acme
		#wget http://11.24.201.36:8080/ky/acme.war
		wget $downloadUrl
		#cp acme.war /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps
		...
		# 6. 删掉之前的配置文件
		cd /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps/acme/WEB_INF/classes/
		rm -rf application-test.properties*


		# 7. 替换原来的war中的配置文件  (2个文件)

		cd /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps/serverSetting

		cp application-test.properties* /acme_dbgo/tomcat8/apache-tomcat-8.5.29/webapps/acme/WEB-INF/classes/
		...
		# 9. 查看启动日志
		tail -f /acme_dbgo/tomcat8/apache-tomcat-8.5.29/logs/catalina.out

elif [ "notOK" -eq $OK_notOK ]	
then	
	echo "不要发版，请注意不要发版！！！"
	exit;
else
