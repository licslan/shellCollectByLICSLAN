   ...
   then
		 ...
		 if [ $BB = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$redis_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $BB = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$redis_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
   ...
   then
		 ...
		 if [ $CC = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$mongodb_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $CC = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$mongodb_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
   ...
   then
		 ...
		 if [ $DD = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$zookeeper_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $DD = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$zookeeper_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
   ...
   then
		 ...
		 if [ $DD = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$nginx_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $DD = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$nginx_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
   ...
   then
		 ...
		 if [ $EE = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$vertx_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $EE = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$vertx_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
   ...
   then
		 ...
		 if [ $FF = $stoping ];
		 	then
				...
				sleep 5s
		 	    # 停止后 并向远程服务器发送请求 告知服务器服务停止成功
		 	    curl -d "ipaddr=$ipaddr&name=$mysql_name&status=2" $urlSuccess
		 else
   				echo "没有符合的条件--------------"
		 fi	    
    else 
		   ...
		   if [ $FF = $starting ];
		 	then
				...
				# 停顿5秒钟
				sleep 5s
				curl -d "ipaddr=$ipaddr&name=$mysql_name&status=1" $urlSuccess
		  else
             echo "没有符合的条件"
          fi
 fi
